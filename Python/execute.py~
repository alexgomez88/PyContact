...
n=50
m=10
T1=np.mat(np.zeros((n,m)))
T2=np.mat(np.zeros((n,m)))
T3=np.mat(np.zeros((n,m)))
T4=np.mat(np.zeros((n,m)))
T5=np.mat(np.zeros((n,m)))
for k in range(0,m):
...
T=np.concatenate((T1.H,T2.H,T3.H,T4.H,T5.H)).H

np.savetxt("ResultadosPython.csv", T, delimiter=',')
